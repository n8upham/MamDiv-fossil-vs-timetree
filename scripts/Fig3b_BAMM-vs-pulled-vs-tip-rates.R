# Code to summarized the calculated pulled speciation rates
#########
# For Fig 3B of new conception Current Biology
#######
...
dev.off()
   ...
   #    # calc MEDIAN of LTT
   #    allLTT_ALL<-do.call(rbind, allLTT)
   #    colnames(allLTT_ALL)<-res$age_grid
   #    allLTT_ALL_Med<-colMedians(allLTT_ALL)
   #    # calc the MEDIAN of PSR
   #    allPSR_ALL<-do.call(rbind, allPSR)
   #    colnames(allPSR_ALL)<-res$age_grid
   #    allPSR_ALL_Med<-colMedians(allPSR_ALL)

    #LAMBDA_bySQS_MED<-list()

        # for median
      #  totalExtantLineages_MED <- allLTT_ALL_Med[1:23]
      #  totalMissingLineages_MED <- c(0,resSQS[c(1:22),"subsampled.richness"])
      #  ALL_lineages_MED <- totalExtantLineages_MED + totalMissingLineages_MED
      #  fractionMissingLineages_MED <- totalMissingLineages_MED / ALL_lineages_MED

      # LAMBDA_bySQS_MED[[q]] <- c(getLambda(PSR = (allPSR_ALL_Med[1:23]), E = fractionMissingLineages_MED), q=as.numeric(paste0("0.",q)) )
...
res_HBD <- fit_hbd_model_on_grid(tree =  mamPhy100[[1]], 
                           oldest_age        = 110,
                           age0              = 0,
                           age_grid          = seq(0, 110, by=10),
                           min_lambda        = 0,
                           max_lambda        = +Inf,
                           min_mu            = 0,
                           max_mu            = +Inf,
      ...
      # plot fitted & true mu
       ...
       mu_fun = approxfun(x=res_HBD$age_grid, y=res_HBD$fitted_mu)



# calculate Pulled Speciation Rate (PSR)
###
# on a grid
#####

root<-round(max(branching.times(mamPhy100[[1]])),0)

res_PSR<-fit_hbd_psr_on_grid(  tree = mamPhy100[[1]], 
                           oldest_age            = 110,
                           age0                  = 0,
                           age_grid              = seq(0, 110, by=10),
                           min_PSR               = 0,
		...
		pdf(file=paste0("fittedRates_2part_tree",i,".pdf"), height=5, width=5*2)
		par(mfcol=c(1,2))
		        plot( x     = res_PSR$age_grid,
		             y     = res_PSR$fitted_LTT,
		             main  = 'Fitted dLTT',
		             xlab  = 'age',
		             ylab  = 'lineages',
		             type  = 'b',
		 ...
		 #     plot( x     = res_PDR$age_grid,
		 #            y     = res_PDR$fitted_PDR,
		 #            main  = 'Fitted PDR',
		 #            xlab  = 'age',
		 #            ylab  = 'PDR',
		 #            type  = 'b',
		 #            xlim  = c(110,0)) 
		     ...
		     polygon(x = c(res_PSR$age_grid, rev(res_PSR$age_grid)), y= c(res_PSR$CI95upper, res_PSR$CI95lower), col="grey")
...
res_PDR <- fit_hbd_pdr_on_grid(  tree = mamPhy100[[1]], 
                           oldest_age            = 110,
                           age0                  = 0,
                           age_grid              = seq(0, 110, by=10),
                           min_PDR               = -1,
      ...
      plot( x     = res_PDR$age_grid,
       ...
       PDR_fun = approxfun(x=res_PDR$age_grid, y=res_PDR$fitted_PDR)




plot(res$fitted_PSR ~ (-res$age_grid), type="l")
