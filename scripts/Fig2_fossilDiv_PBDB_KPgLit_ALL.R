...
# load back
    ...
    >> Then use the minMax extinction rates to re-calc the HBD pushed speciation rates
        >> then REPLOT !
...
# Get the perCapita rates now:
####
    # set intervals (setup in reverse order)
    # generate 10-Ma bins
  #  bins_10Ma<- seq(from=0,to=130,by= 10)
  #  binNames_10Ma<-paste0("bin_",bins_10Ma)[-14]
    bins_5Ma<- seq(from=0,to=130,by= 5)
    binNames_5Ma<-paste0("bin_",bins_5Ma)[-27]
  #  bins_1Ma<- seq(from=0,to=130,by= 1)
  #  binNames_1Ma<-paste0("bin_",bins_1Ma)[-131]
    # load STAGES
  #  stages<-read.csv(file="stagesThroughTime.csv", header=TRUE)
    
  #  presetIntervals_10Ma <- cbind(rev(bins)[-1]+10, rev(bins)[-1])
  #  rownames(presetIntervals_10Ma)<-rev(binNames)
    presetIntervals_5Ma <- cbind(rev(bins_5Ma)[-1]+5, rev(bins_5Ma)[-1])
    rownames(presetIntervals_5Ma)<-rev(binNames_5Ma)
  #  presetIntervals_1Ma <- cbind(rev(bins_1Ma)[-1]+1, rev(bins_1Ma)[-1])
  #  rownames(presetIntervals_1Ma)<-rev(binNames_1Ma)
  #  presetIntervals_stages <- cbind(stages$max_ma, stages$min_ma)
    
    # get binned counts
  #  genRanges_10Ma <- binTimeData(readyToBin, int.times = presetIntervals_10Ma)
    genRanges_5Ma <- binTimeData(readyToBin, int.times = presetIntervals_5Ma)
  #  genRanges_1Ma <- binTimeData(readyToBin, int.times = presetIntervals_1Ma)
  #  genRanges_stages <- binTimeData(readyToBin, int.times = presetIntervals_stages)

    # calculate rates
  #  perCapRates_10Ma <- perCapitaRates(genRanges_10Ma)
    perCapRates_5Ma <- perCapitaRates(genRanges_5Ma)
  #  perCapRates_1Ma <- perCapitaRates(genRanges_1Ma)
    perCapRates_stages <- perCapitaRates(genRanges_stages)
        rownames(perCapRates_stages)<-stages$interval_name

        # confirming the pRate = origination, qRate = extinction
        #origination = -log((perCapRates_10Ma[,"Nbt"])/(perCapRates_10Ma[,"Nbt"]+perCapRates_10Ma[,"NFt"]))/perCapRates_10Ma[,"intlen"] # divided by the change in time (length of interval)
        #extinction = -log((perCapRates_10Ma[,"Nbt"])/(perCapRates_10Ma[,"Nbt"]+perCapRates_10Ma[,"NbL"]))/perCapRates_10Ma[,"intlen"] # divided by the change in time (length of interval)
        #rateDat<-cbind.data.frame(perCapRates_10Ma[,1:2],origination,extinction)

    # SAVE the perCapRates files
    #######
    write.csv(perCapRates_5Ma, file="fossilGeneraThroughTime_PBDB_perCapitaRates_bins5Ma.csv")
    write.csv(perCapRates_10Ma, file="fossilGeneraThroughTime_PBDB_perCapitaRates_bins10Ma.csv")
    write.csv(perCapRates_stages, file="fossilGeneraThroughTime_PBDB_perCapitaRates_binsStages.csv")
    write.csv(perCapRates_1Ma, file="fossilGeneraThroughTime_PBDB_perCapitaRates_bins1Ma.csv")


    # plot the perCapita rates through time
    ####
    Lwds<-2
    exclVal<-c(47:48)
    #rateDat <- perCapRates_stages[-exclVal,]
    rateDat <- perCapRates_5Ma
    q <- 5
    SQSdat <- read.csv(file=paste0("tables_DivTT_RateTT/SQSresults_PBDB_quorum0p",q,"_bins5Ma.csv"))
    SQStime <- seq(5,130,by=5)

    pdf(file="fossilGeneraThroughTime_PBDB_SQS-0p5_fullDat_5Ma-manual_wRates-5Ma_cols.pdf",height=5, width=7)

    par(oma = c(2,2,2,2)+0.1, mar = rep(2,4) + 0.1)

    plot(SQSdat[,"raw.richness"] ~ SQStime, type="n", xlim=c(120,0), log="y", ylim=c(1,2000), yaxt="n",bty="n",ylab="", xlab="")
    axis(side=2, at=c(5,20,200,2000))
    mtext(side=2,text="Fossil genera (log)",line=2.5, adj=0.85)
    mtext(side=1,text="Million years before present (Ma)",line=2.5, adj=0.5)

    abline(v=66, lty=2, lwd=3, col="grey")
    points(SQSdat[,"raw.richness"] ~ SQStime, type="l", lwd=Lwds)#, xlim=c(110,0), log="y", ylim=c(5,2000))
    points(SQSdat[,"subsampled.richness"] ~ SQStime, type="l", lty=2, lwd=Lwds, col="black")#, xlim=c(110,0), log="y", ylim=c(5,200))

    par(new=TRUE)
    plot(rateDat[,"pRate"] ~ rateDat[,1], type="l", lwd=8, col="red", xlim=c(120,0), ylim=c(-0,3.5), bty="n",yaxt="n",xaxt="n",ylab="",xlab="")
    points(rateDat[,"pRate"] ~ rateDat[,1], type="l", lwd=2, col="pink", xlim=c(120,0), ylim=c(-0,3.5), bty="n",yaxt="n",xaxt="n",ylab="",xlab="")
    points(rateDat[,"qRate"] ~ rateDat[,1], type="l", lwd=8, col=rgb(0,0,1,alpha=0.3))#, xlim=c(110,0), log="y", ylim=c(5,200))
    points(rateDat[,"qRate"] ~ rateDat[,1], type="l", lwd=2, col="blue")#, xlim=c(110,0), log="y", ylim=c(5,200))
    axis(side=4, at=c(0,1,2))
    mtext(side=4,text="Rate (genera / Ma)",line=2.5, adj=0.1)
   ...
   # legend(x=110.5, y=1.5, legend=c("total diversity","subsampled diversity","origination rate","extinction rate"),cex=0.8,lty=c(1,2,1,1),col=c(1,1,"blue","red"),lwd=rep(Lwds,4))
    ...
    # output those rates to a TABLE (for use in the pulled rate corrections.)
    ####
    ...
    # use paleotree binTimeData() function to do binning
    # generate 10-Ma bins
    bins<- seq(from=240,to=10,by= -10)
    binNames<-paste0("bin_",bins)

    # use paleotree binTimeData() function to do binning
    presetIntervals <- cbind(bins, bins-10)
    rownames(presetIntervals)<-binNames
    genRanges <- binTimeData(readyToBin, int.times = presetIntervals)

    lastMinusFirst<-genRanges[[2]][,2]-genRanges[[2]][,1]
    for(k in 1:length(lastMinusFirst)){
        if(lastMinusFirst==0){
            <- binNames[genRanges[[2]][k,2]]
        }

    }


    #taxicDivDisc(genRanges)
    perCapRates_10Ma <- perCapitaRates(genRanges)

    # join bins back to genus data
    genBinned <- cbind.data.frame(readyDat, genRanges[[2]])
    
    # simplify
    first<-genBinned[,c("accepted_name","first.int")]
    colnames(first)<-c("genus","int")
    last<-genBinned[,c("accepted_name","last.int")]
    colnames(last)<-c("genus","int")
    genBinned_BOTH <- rbind( first, last)
    genBinned_BOTH_uniq <- unique(genBinned_BOTH)

    numBins<-rev(names(table(genBinned_BOTH$int)))

    # do the SQS subsampling
    sqsGenusDiv_new<-vector("list",length(numBins))
    for(i in 1:length(numBins)){
        bin<-numBins[i]
        t = table(genBinned_BOTH[which(genBinned_BOTH$int==bin),"genus"])
        sqsGenusDiv_new[[i]]<-sqs(ab=t[t != 0],q=0.5,trials=1000, ignore.singletons=FALSE)
    }
    sqsGenusDiv_new_ALL<-do.call(rbind,sqsGenusDiv_new)
    rownames(sqsGenusDiv_new_ALL)<-numBins
...
genOccs<-read.csv(file="fossilGen_throughTime_5428gen_Mar2020_wHigher.csv")


# Original code from Aug 2018...
#####


o = read.table("Nate-occs.csv",header=T,sep=",")
binNames <- names(table(o$X10_my_bin))[-1][1:14]

# get the subsamplesd richnesses through time-- as based on the 10-MY-BINS
######
rawGenusDiv<-c()
sqsGenusDiv<-vector("list",length(binNames))
for(i in 1:length(binNames)){
	bin<-binNames[i]
	t = table(o[which(o$X10_my_bin==bin),"occurrence.genus_name"])
	rawGenusDiv[i]<-length(t[t != 0])
	sqsGenusDiv[[i]]<-sqs(ab=t[t != 0],q=0.5,trials=1000, ignore.singletons=FALSE)
}
sqsGenusDiv_ALL<-do.call(rbind,sqsGenusDiv)
rownames(sqsGenusDiv_ALL)<-binNames

sqsGenusDiv_ALL[,1]<-seq(from=0,to=130,by=10)
colnames(sqsGenusDiv_ALL)<-c("Time",colnames(sqsGenusDiv_ALL)[-1])

# now plot!
plot(sqsGenusDiv_ALL[,"raw richness"] ~ sqsGenusDiv_ALL[,"Time"], type="n", xlim=c(110,0), log="y", ylim=c(5,2000), yaxt="n",bty="n",ylab="Fossil diversity (genera)", xlab="Million years before present (Ma)")
axis(side=2, at=c(5,20,50,200,500,2000))
abline(v=66, lty=2, lwd=2, col="grey")
points(sqsGenusDiv_ALL[,"raw richness"] ~ sqsGenusDiv_ALL[,"Time"], type="l", lwd=2)#, xlim=c(110,0), log="y", ylim=c(5,2000))
points(sqsGenusDiv_ALL[,"subsampled richness"] ~ sqsGenusDiv_ALL[,"Time"], type="l", lwd=2, col="red")#, xlim=c(110,0), log="y", ylim=c(5,200))

#plot(sqsGenusDiv_ALL[,"specimens"] ~ sqsGenusDiv_ALL[,"Time"], type="l", xlim=c(110,0), log="y", ylim=c(5,22000), bty="n",ylab="Fossil diversity (genera)", xlab="Million years before present (Ma)")


# get the rates through time-- as based on the STAGES
######
divThroughTime<-read.csv("pbdb_data_diversity-over-time_regularTaxa.csv", skip=23, header=TRUE)
#origination = -ln((xbt)/(xbt+xft))/(eag-lag) # divided by the change in time (length of interval)
#extinction = -ln((xbt)/(xbt+xbl))/(eag-lag) # divided by the change in time (length of interval)

datDatForRates<-cbind.data.frame(divThroughTime[,2:8],intLength=(divThroughTime[,"max_ma"]-divThroughTime[,"min_ma"]))

origination = -log((datDatForRates[,"X_bt"])/(datDatForRates[,"X_bt"]+datDatForRates[,"X_Ft"]))/datDatForRates[,"intLength"] # divided by the change in time (length of interval)
extinction = -log((datDatForRates[,"X_bt"])/(datDatForRates[,"X_bt"]+datDatForRates[,"X_bL"]))/datDatForRates[,"intLength"] # divided by the change in time (length of interval)

rateDat<-cbind.data.frame(divThroughTime[,2:4],origination,extinction)

# Do the full plot now.
######
exclVal<-2
Lwds<-2
pdf(file="fossilGeneraThroughTime_PBDB_SQSrichness_perCapitaRates.pdf",height=5, width=7)

par(oma = c(2,2,2,2)+0.1, mar = rep(2,4) + 0.1)

plot(sqsGenusDiv_ALL[,"raw richness"] ~ sqsGenusDiv_ALL[,"Time"], type="n", xlim=c(110,0), log="y", ylim=c(1,2000), yaxt="n",bty="n",ylab="", xlab="")
axis(side=2, at=c(5,20,200,2000))
mtext(side=2,text="Fossil diversity (genera / 10 Ma)",line=2.5, adj=0.85)
mtext(side=1,text="Million years before present (Ma)",line=2.5, adj=0.5)

abline(v=66, lty=2, lwd=3, col="grey")
points(sqsGenusDiv_ALL[,"raw richness"] ~ sqsGenusDiv_ALL[,"Time"], type="l", lwd=Lwds)#, xlim=c(110,0), log="y", ylim=c(5,2000))
points(sqsGenusDiv_ALL[,"subsampled richness"] ~ sqsGenusDiv_ALL[,"Time"], type="l", lty=2, lwd=Lwds, col="black")#, xlim=c(110,0), log="y", ylim=c(5,200))
...
allMamCollections<-read.csv("pbdb_data_collections-lumpByGenus-regularTaxa.csv", skip=19, header=TRUE)

Coll_freqOfMin_ALL<-table(allMamCollections[,"min_ma"])


Div_stages<-as.character(divThroughTime[,"interval_name"])


Coll_freqOfMin_Stages<-table(allMamCollections[match(allMamCollections$early_interval,Div_stages),"min_ma"])


Coll_perDivStage<-c()
for(i in 1:length(Div_stages)){
	Stage<-Div_stages[i]
	Coll_perDivStage[i]<-length(allMamCollections[which(allMamCollections$early_interval==Stage),1])
}

dataMod<-cbind.data.frame(divThroughTime, numColl=Coll_perDivStage, totalSampBin=(divThroughTime$sampled_in_bin+divThroughTime$implied_in_bin),numSampBin_perColl=((divThroughTime$sampled_in_bin+divThroughTime$implied_in_bin)/Coll_perDivStage))


plot(totalSampBin ~ min_ma, data=dataMod, xlim=c(100,0), type="l", log="y", ylim=c(5,1000))



library(paleotree) # load the library set.seed(1) # set random seed
taxa <- simFossilRecord(p=0.1,q=0.1)



The diversity curve functions used include: ‘phyloDiv’,
     ‘taxicDivCont’ and ‘taxicDivDisc’.
...
rangesCont <- sampleRanges(taxa,r=0.2) # plot figure 1B taxicDivCont(rangesCont,
int.times=taxadiv[,1:2]) title(c("Sampled Diversity Curve",
"(Continuous Time)"))


maxtime=1000,maxExtant=0)
# plot true diversity curve as Fig. 1A layout(1:3);par(mar=c(4Æ5,4,4,1))
taxadiv <- taxicDivCont(taxa)
# saves div curve data as ‘taxadiv’ title("True Diversity Curve")